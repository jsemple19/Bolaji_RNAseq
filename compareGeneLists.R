...
recoveryol<-data.frame(NumPrograms=1:max(ortholist$No..of.Programs),upGenes=NA,downGenes=NA)
nrow(sigUp)
for(i in 1:nrow(recoveryol)){
  print(paste0("using ",i," programs"))
  ij<-inner_join(as.data.frame(sigUp),ortholist[ortholist$No..of.Programs>=i,],by=join_by("Wormbase.ID"=="WormBase.ID"))
  recoveryol$upGenes[i]<-round(length(unique(ij$Wormbase.ID))/nrow(sigUp),2)
}

nrow(sigDown)
for(i in 1:max(ortholist$No..of.Programs)){
  print(paste0("using ",i," programs"))
  ij<-inner_join(as.data.frame(sigDown),ortholist[ortholist$No..of.Programs>=i,],by=join_by("Wormbase.ID"=="WormBase.ID"))
  recoveryol$downGenes[i]<-round(length(unique(ij$Wormbase.ID))/nrow(sigDown),2)
}

numProgs<-1
sigHS<-list()
sigHS[["COH-1cs.up"]]<-unique(inner_join(as.data.frame(sigUp), ortholist[ortholist$No..of.Programs>=numProgs,],by=join_by("Wormbase.ID"=="WormBase.ID"))$HGNC.Symbol)
sigHS[["COH-1cs.down"]]<-unique(inner_join(as.data.frame(sigDown), ortholist[ortholist$No..of.Programs>=numProgs,],by=join_by("Wormbase.ID"=="WormBase.ID"))$HGNC.Symbol)
sigHS[["SCC-1cs.up"]]<-unique(inner_join(as.data.frame(scc1Up), ortholist[ortholist$No..of.Programs>=numProgs,],by=join_by("Wormbase.ID"=="WormBase.ID"))$HGNC.Symbol)
sigHS[["SCC-1cs.down"]]<-unique(inner_join(as.data.frame(scc1Down), ortholist[ortholist$No..of.Programs>=numProgs,],by=join_by("Wormbase.ID"=="WormBase.ID"))$HGNC.Symbol)
...
head(cdl)
idx<-cdl$Gene.Symbol %in% ortholist$HGNC.Symbol[ortholist$No..of.Programs>=numProgs] #6008 for 3progs from 17309
cdl<-cdl[idx,]
...
#' @return plot
#' @export
plotEnrichment1<-function (pathway, stats, gseaParam = 1, ticksSize = 0.2){
  ...
  g+labs(title=paste0("CdLS enrichment of ",pathName, " genes")) +
...
for (pathName in unlist(fgseaRes[,"pathway"])) {
  gseaList[[pathName]]<-plotEnrichment1(sigHS[[pathName]], ranks) #+
...
p<-gridExtra::marrangeGrob(grobs=gseaList, nrow=3, ncol=1,top=paste0("orthologs found with at least ",numProgs," programs"))
print(p)
dev.off()
...
sigHS[["COH-1cs.up"]]<-unique(inner_join(as.data.frame(sigUp), ortholist[ortholist$No..of.Programs>=numProgs,],by=join_by("Wormbase.ID"=="WormBase.ID"))$HGNC.Symbol)
sigHS[["COH-1cs.down"]]<-unique(inner_join(as.data.frame(sigDown), ortholist[ortholist$No..of.Programs>=numProgs,],by=join_by("Wormbase.ID"=="WormBase.ID"))$HGNC.Symbol)
sigHS[["random"]]<-sample(ortholist$HGNC.Symbol[ortholist$No..of.Programs>=numProgs],700)
...
expr.input<-cdl[row.names(cdl) %in% ortholist$HGNC.Symbol[ortholist$No..of.Programs>=numProgs],]
dim(expr.input)
...
p <- ggplot(data1, aes(iidx,RES))+geom_line(col="black")
