...
sampleTable<-data.frame(fileName=fileNames,sampleName=sampleNames,
                        sampleGroup,replicate)

sampleTable<-sampleTable[grep("^T",sampleTable$sampleName,invert=T),]
...
highlight<-c(metadata$wormbaseID[metadata$publicID %in% c("top-1","top-2","ama-1")])
names(highlight)<-c(metadata$publicID[metadata$publicID %in% c("top-1","top-2","ama-1")])
...
contrastsOI<-list(AMAvsN2="sampleGroup_AMA_vs_N2",FLAVOvsN2="sampleGroup_FLAVO_vs_N2")
