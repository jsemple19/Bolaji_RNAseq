...
#' Get library size
#'
#' Use quant.sf files to get library size for each sample
#' @param sf_dirs vector of directories with location of quant.sf file
#' @return data.frame with path to quant.sf file and total counts of that sample
#' @export
getLibSizes<-function(sf_dirs){
  df<-data.frame(path=sf_dirs,size=NA)
  for (i in 1:nrow(df)){
    tb<-read.table(paste0(sf_dirs[i],"/quant.sf"),header=T)
    df$size[i]<-sum(tb$NumReads)
  }
  return(df)
}
