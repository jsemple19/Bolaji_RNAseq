...
####
namePrefix <- "coh1_noOsc"  #use _noOsc in prefix to automatically filter oscillating genes
...
sampleNames<-fileList$sampleName

fileNames<-paste0(outPath,"/salmon/mRNA/",sampleNames,"/quant.sf")

sampleTable<-data.frame(fileName=fileNames,sampleName=factor(sampleNames),
                        strain=factor(fileList$strain),repeatNum=factor(fileList$repeatNum),
                        laneNum=factor(fileList$laneNum),date=factor(fileList$date))
...
highlight<-c(metadata$wormbaseID[metadata$publicID %in% c("top-1","top-2","ama-1","coh-1")])
names(highlight)<-c(metadata$publicID[metadata$publicID %in% c("top-1","top-2","ama-1","coh-1")])
...
modelTxt<-"~ laneNum + repeatNum  + strain"
...
contrastsOI<-list(COH1vsTEVonly="strain_828_vs_366")
...
for(grp in contrastNames){
  ...
  if(is.character(contrastsOI[[grp]])){
    res<-results(dds,name=contrastsOI[[grp]],alpha=padjVal)
  } else {
